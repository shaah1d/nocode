...
export async function GET(request: Request, { params }: { params: { hyperModel: string } }) {
  try {
    // Extract the modelName from the dynamic route parameter
    const { hyperModel } = params;

    // Construct the path to the Python script dynamically
    const pythonScriptPath = path.join(process.cwd(), `hyperpymodules/${hyperModel}.py`);
