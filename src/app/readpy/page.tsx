...
export default function Home() {
  ...
  const [modelName, setModelName] = useState<string>(''); // No default model name
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [modelName]); // Re-run the effect when modelName changes

  const handleModelChange = (event: any) => {
    setError(null)
    setModelName(event.target.value); // Update the selected model name
  };
